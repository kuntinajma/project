...
export async function updateDestination(
  ...
  try {
    console.log("Updating destination with token:", token?.substring(0, 10) + "...");
    
    if (!token) {
      console.error("No token provided for updateDestination");
      return {
        success: false,
    ...
    console.log("Update destination response:", res);
    return res;
  } catch (err: any) {
    console.error("Update destination failed:", err);
    if (err.response) {
