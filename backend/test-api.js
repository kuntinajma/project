...
const testUpdateProfile = async () => {
  ...
  const result = await apiRequest('PUT', '/auth/profile', updateData, authToken);
...
const testAuthorization = async () => {
  ...
  if (!result.success && result.status === 403) {
    logSuccess('Authorization working correctly! Contributor cannot access user management.');
    logInfo('Expected 403 Forbidden error received');
  } else if (result.success) {
    logError('Authorization failed! Contributor should not access user management.');
  } else {
...
const testSuperadminFunctions = async () => {
  ...
  const registerResult = await apiRequest('POST', '/auth/register', superadminData);
  ...
  if (usersResult.success) {
    ...
    logInfo(`Users: ${usersResult.data.data.users.map(u => `${u.name} (${u.role})`).join(', ')}`);
  } else {
...
const runAllTests = async () => {
  console.log(`${colors.bold}${colors.blue}🚀 Starting Laiya Tourism Backend API Tests${colors.reset}\n`);
  ...
  console.log(`${colors.bold}Passed: ${colors.green}${passedTests}${colors.reset}${colors.bold}/${totalTests}${colors.reset}`);
